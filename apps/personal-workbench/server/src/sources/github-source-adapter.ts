...
export class GitHubSourceAdapter implements KnowledgeSourceAdapter {
  ...
  async acquire(source: DetectedKnowledgeSource, context: SourceAdapterContext): Promise<UnifiedDocumentRecord> {
    ...
    try {
      ...
      const clone = await this.execute(executable, githubCloneArgs(parsed.canonical, repoRoot), { cwd: jobRoot, timeoutMs: 90_000 })
      ...
      if (!artifactBelongsToRoot(canonicalRoot, jobRoot)) throw new SourceAdapterError('GITHUB_REPOSITORY_TRAVERSAL_DENIED', '仓库目录超出了受控范围，已经停止分析。')
