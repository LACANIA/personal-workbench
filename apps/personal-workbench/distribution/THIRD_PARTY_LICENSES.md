...
本清单覆盖 Windows 发行版直接包含的生产依赖，以及首次运行界面能够检测、但不会随安装包分发的可选组件。各项目的完整许可文本随其 npm 包或官方分发包保留。
...
| PDF.js (`pdfjs-dist` 6.3.289) | 没有外部 `pdftotext` 时进行本机 PDF 文本提取 | 随应用分发 | Apache-2.0 |
| `@napi-rs/canvas` | PDF.js 可选本机渲染依赖 | 随 npm 生产依赖分发 | MIT |
| DeepSeek Harness 0.1.0-rc.5 | 本机任务运行时 | 仅分发生产运行闭包，官方 Git 源码仓库未打包 | MIT |
| personal-safe-fs | Harness 只读文件工具 | 随生产运行闭包分发 | 本项目私有组件 |
...
Personal Workbench 不会把 Ollama 模型、开发数据库、个人输出或测试资料放入安装包。
